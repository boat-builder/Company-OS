...
`salesx` is a `uv`-managed CLI that lives at a fixed path and loads its own credentials
from a `.env` beside it — you do **not** set any environment variables yourself.

**Always invoke it like this** (the `cd` matters — that is how it finds its `.env`):

```bash
cd /Users/hhsecond/asgard/salesx && UV_LINK_MODE=copy uv run salesx <group> <command> [flags]
```

In a Cowork session the same directory is reachable from the shell at
`/sessions/*/mnt/asgard/salesx` — use whichever path your shell resolves; the
`company-os` mount is the same folder. The first `uv run` may build a venv (a few
