...
- **Pricing principle (the anchor):** Berlin's price for any given customer is anchored at approximately **one-quarter of what an agency would charge for the same scope of work.** Scope determines the absolute number; the agency-fraction anchor stays constant. The 4-tier scope ladder and agency channel discount band were removed from canonical on 2026-07-28 pending re-derivation for the AEO direction (see `product.md` → *Pricing*). **The agency-fraction anchor itself is also open** — it assumes an agency-equivalent SEO scope to divide against, which an AEO-exclusive product may not have. Projections downstream of this anchor should be treated as provisional until pricing is rebuilt.
