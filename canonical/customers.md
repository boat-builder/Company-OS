...
This follows the June 2026 decision to double down on the co-equal **platform + FDM** model and position Berlin in the **middle of the tool↔agency spectrum**. ⚠️ That spectrum framing was removed from canonical on 2026-07-28 along with the rest of positioning, pending re-derivation for the AEO direction (see `product.md` → *Positioning*) — treat the reasoning below as the historical basis for the ICP, not as current positioning. The SEO-literate buyer is exactly the middle of that spectrum:
...
Pricing for this segment is being re-derived alongside the ICP and is not yet fixed. SaaS buyers in this band span a founder-led tier and a marketing-leader tier; expect roughly **$499–$999/mo** where a hands-on founder owns SEO, and **$2,500–$5,000/mo** where a marketing leader with a team and an existing budget owns it. Treat these as anchors to test, not settled tiers. There is currently **no canonical pricing file** — pricing was removed from canonical on 2026-07-28 pending re-derivation for the AEO direction (see `product.md` → *Pricing*), so the anchors above are the closest thing to a live reference and remain untested. Sales-side pricing discipline lives in `sales.md`.
