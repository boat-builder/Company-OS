...
In Berlin's product terminology, *audit* is the **continuous observability mode**. Berlin draws from the **strategy knowledge base** (see Engineering Moat) — a vetted library of SEO/AEO strategy patterns maintained by Berlin's in-house research team — and selects a brand-shaped portfolio of *workflows*. Each workflow is a scheduled watch job that polls a slice of the brand's SEO surface, brings back signal, and re-runs on its own cadence.
...
| **Continuous Audit** | Observability mode. Berlin runs a brand-shaped portfolio of scheduled workflows drawn from the **strategy knowledge base** (see Tools Layer table below), each polling a slice of the brand's SEO surface on its own cadence. | Live |
